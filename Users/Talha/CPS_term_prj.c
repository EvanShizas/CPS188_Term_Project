...
//TALHA CODE
	...
	/*					QUESTION 2						*/
	puts("Question 2: ");
    double maxPercent_diabetes = -100;
    double minPercent_diabetes = 100;
    char* maxProvince;
    char* minProvince;
    
    /////////////// Getting Province with MAX % ///////////////
    
    if (maxPercent_diabetes < sumON / cON) {
        maxPercent_diabetes = sumON / cON;
        maxProvince = "Ontario";
    }
    if (maxPercent_diabetes < sumQC / cQC) {
        maxPercent_diabetes = sumQC / cQC;
        maxProvince = "Quebec";
    }
    if (maxPercent_diabetes < sumBC / cBC) {
        maxPercent_diabetes = sumBC / cBC;
        maxProvince = "British Columbia";
    }
    if (maxPercent_diabetes < sumAB / cAB) {
        maxPercent_diabetes = sumAB / cAB;
        maxProvince = "Alberta";
    }
    
    /////////////// Getting Province with MIN % /////////////// 
    
    if (minPercent_diabetes > sumON / cON) {
        minPercent_diabetes = sumON / cON;
        minProvince = "Ontario";
    }
    if (minPercent_diabetes > sumQC / cQC) {
        minPercent_diabetes = sumQC / cQC;
        minProvince = "Quebec";
    }
    if (minPercent_diabetes > sumBC / cBC) {
        minPercent_diabetes = sumBC / cBC;
        minProvince = "British Columbia";
    }
    if (minPercent_diabetes > sumAB / cAB) {
        minPercent_diabetes = sumAB / cAB;
        minProvince = "Alberta";
    }
    
    printf("Province with the HIGHEST percentage of Diabetes is %s with %.2lf\n", maxProvince, maxPercent_diabetes);
    printf("Province with the LOWEST percentage of Diabetes is %s with %.2lf\n", minProvince, minPercent_diabetes);
    
	/*					QUESTION 3						*/
	puts("\nQuestion 3: ");
	double nationalAVG = sumNA/ cNA;

	/////////////// over the national average ///////////////
	
	if (nationalAVG < sumON / cON) {
        printf("Ontario is above the national average for percentage of diabetes\n");
    }
    if (nationalAVG < sumQC / cQC) {
        printf("Quebec is above the national average for percentage of diabetes\n");
    }
    if (nationalAVG < sumBC / cBC) {
        printf("British Columbia is above the national average for percentage of diabetes\n");
    }
    if (nationalAVG < sumAB / cAB) {
        printf("Alberta is above the national average for percentage of diabetes\n");
    }
    /////////////// under the national average ///////////////
    
	if (nationalAVG > sumON / cON) {
        printf("Ontario is below the national average for percentage of diabetes\n");
    }
    if (nationalAVG > sumQC / cQC) {
        printf("Quebec is below the national average for percentage of diabetes\n");
    }
    if (nationalAVG > sumBC / cBC) {
        printf("British Columbia is below the national average for percentage of diabetes\n");
    }
    if (nationalAVG > sumAB / cAB) {
        printf("Alberta is below the national average for percentage of diabetes\n");
    }
